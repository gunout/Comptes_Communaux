...
class TamponFinanceAnalyzer:
    def __init__(self):
        self.commune = "Le Tampon"
    ...
    def generate_financial_data(self):
        """Génère des données financières pour la commune du Tampon"""
        print("🏛️ Génération des données financières pour Le Tampon...")
        ...
        data['Investissement_Agricole'] = self._simulate_agricultural_investment(dates)
        
        df = pd.DataFrame(data)
        
        # Ajouter des tendances spécifiques au Tampon
        self._add_municipal_trends(df)
        
        return df
    
    def _simulate_population(self, dates):
        """Simule la population du Tampon"""
        base_population = 65000  # population estimée en 2002
    ...
    def _simulate_households(self, dates):
        ...
        base_households = 22000  # ménages en 2002
    ...
    def _simulate_total_expenses(self, dates):
        ...
        base_expenses = 52  # millions d'euros en 2002
    ...
    def _simulate_staff_costs(self, dates):
        ...
        base_staff = 20  # millions d'euros en 2002
    ...
    def _simulate_gross_savings(self, dates):
        ...
        for i, date in enumerate(dates):
            ...
            base_saving = 4  # millions d'euros en 2002
    ...
    def _simulate_total_debt(self, dates):
        ...
        base_debt = 40  # millions d'euros en 2002
    ...
    def _simulate_agricultural_investment(self, dates):
        """Simule l'investissement agricole (spécifique au Tampon)"""
        base_investment = 2.0  # millions d'euros en 2002
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2007, 2013, 2019, 2023]:  # Gros investissements agricoles
                multiplier = 2.0
            else:
                multiplier = 1.0
    ...
    def _add_municipal_trends(self, df):
        """Ajoute des tendances municipales réalistes pour Le Tampon"""
        for i, row in df.iterrows():
            ...
            # Développement agricole (spécifique au Tampon)
            if year >= 2010:
                agricultural_growth = 1 + 0.025 * (year - 2010)
                df.loc[i, 'Investissement_Agricole'] *= agricultural_growth
            
            # Plan de relance post-COVID (2022-2025)
            if year >= 2022:
                df.loc[i, 'Investissement'] *= 1.1
                df.loc[i, 'Investissement_Equipements'] *= 1.05
                df.loc[i, 'Investissement_Agricole'] *= 1.08
    ...
    def create_financial_analysis(self, df):
        ...
        plt.suptitle(f'Analyse des Comptes Communaux du Tampon ({self.start_year}-{self.end_year})', 
                    fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.savefig('tampon_financial_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _plot_investments(self, df, ax):
        ...
        ax.plot(df['Annee'], df['Investissement_Agricole'], label='Agriculture', 
    ...
    def _plot_sectorial_investments(self, df, ax):
        ...
        bottom = np.zeros(len(years))
        categories = ['Investissement_Equipements', 'Investissement_Urbanisme', 
                     'Investissement_Voirie', 'Investissement_Culture', 'Investissement_Agricole']
        colors = ['#264653', '#2A9D8F', '#E76F51', '#F9A602', '#6A0572']
        labels = ['Équipements', 'Urbanisme', 'Voirie', 'Culture', 'Agriculture']
    ...
    def _generate_financial_insights(self, df):
        ...
        print(f"🏛️ INSIGHTS ANALYTIQUES - Commune du Tampon")
        print("=" * 60)
        ...
        # 5. Spécificités du Tampon
        print("\n5. 🌄 SPÉCIFICITÉS COMMUNALES:")
        avg_agricultural_investment = df['Investissement_Agricole'].mean()
        agricultural_share = (df['Investissement_Agricole'].mean() / df['Investissement'].mean()) * 100
        
        print(f"Investissement agricole moyen: {avg_agricultural_investment:.2f} M€")
        print(f"Part de l'agriculture dans l'investissement: {agricultural_share:.1f}%")
        
        # 6. Événements marquants
        print("\n6. 📅 ÉVÉNEMENTS MARQUANTS:")
        print("• 2002-2005: Développement initial de la commune")
        print("• 2007-2008: Développement des infrastructures agricoles")
        print("• 2008-2009: Impact de la crise financière mondiale")
        print("• 2010-2015: Modernisation des équipements communaux")
        print("• 2020-2021: Gestion de la crise COVID-19")
        print("• 2022-2025: Plan de relance post-COVID avec accent sur l'agriculture")
        
        # 7. Recommandations
        print("\n7. 💡 RECOMMANDATIONS STRATÉGIQUES:")
        print("• Maintenir le soutien au secteur agricole, pilier de l'économie locale")
        ...
        print("• Adapter les services aux besoins d'une population vieillissante")
        print("• Développer l'écotourisme pour valoriser le patrimoine naturel")
        print("• Renforcer les partenariats avec les acteurs du développement agricole")
        print("• Poursuivre la modernisation des infrastructures tout en maîtrisant les coûts")

def main():
    """Fonction principale"""
    print("🏛️ ANALYSE DES COMPTES COMMUNAUX DU TAMPON (2002-2025)")
    print("=" * 60)
    
    # Initialiser l'analyseur
    analyzer = TamponFinanceAnalyzer()
    
    # Générer les données
    financial_data = analyzer.generate_financial_data()
    
    # Sauvegarder les données
    output_file = 'tampon_financial_data_2002_2025.csv'
