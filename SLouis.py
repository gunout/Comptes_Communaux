...
class SaintLouisFinanceAnalyzer:
    def __init__(self):
        self.commune = "Saint-Louis"
    ...
    def generate_financial_data(self):
        """Génère des données financières pour la commune de Saint-Louis"""
        ...
        # Données démographiques (ville importante du sud)
        ...
        # Investissements spécifiques (adaptés à Saint-Louis)
        ...
        # Ajouter des tendances spécifiques à Saint-Louis
        self._add_municipal_trends(df)
        
        return df
    
    def _simulate_population(self, dates):
        """Simule la population de Saint-Louis"""
        base_population = 45000  # population estimée en 2002
    ...
    def _simulate_households(self, dates):
        ...
        base_households = 15000  # ménages en 2002
    ...
    def _simulate_total_revenue(self, dates):
        ...
        base_revenue = 45  # millions d'euros en 2002
    ...
    def _simulate_operating_expenses(self, dates):
        ...
        base_operating = 28  # millions d'euros en 2002
    ...
    def _simulate_investment_expenses(self, dates):
        ...
        base_investment = 15  # millions d'euros en 2002
    ...
    def _simulate_staff_costs(self, dates):
        ...
        base_staff = 22  # millions d'euros en 2002
    ...
    def _simulate_agriculture_investment(self, dates):
        """Simule l'investissement agricole (spécifique à Saint-Louis)"""
    ...
    def _simulate_industry_investment(self, dates):
        """Simule l'investissement industriel (spécifique à Saint-Louis)"""
    ...
    def _simulate_commerce_investment(self, dates):
        """Simule l'investissement commercial (spécifique à Saint-Louis)"""
        base_investment = 2.8  # millions d'euros en 2002
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2008, 2013, 2018, 2023]:  # Investissements commerciaux
                multiplier = 1.7
            else:
                multiplier = 1.0
    ...
    def _simulate_education_investment(self, dates):
        ...
        for i, date in enumerate(dates):
            year = date.year
            if year in [2005, 2010, 2015, 2020]:  # Gros investissements éducatifs
                multiplier = 1.6
            else:
                multiplier = 1.0
    ...
    def _simulate_transport_investment(self, dates):
        ...
        base_investment = 3.2  # millions d'euros en 2002
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2009, 2014, 2019, 2024]:  # Investissements en transport
                multiplier = 1.7
            else:
                multiplier = 1.0
    ...
    def _add_municipal_trends(self, df):
        """Ajoute des tendances municipales réalistes pour Saint-Louis"""
        for i, row in df.iterrows():
            year = row['Annee']
            
            # Développement initial (2002-2005)
            if 2002 <= year <= 2005:
                df.loc[i, 'Investissement_Industrie'] *= 1.4
            ...
            if 2020 <= year <= 2021:
                if year == 2020:
                    # Baisse des recettes commerciales et industrielles
                    df.loc[i, 'Autres_Recettes'] *= 0.86
                    df.loc[i, 'Impots_Locaux'] *= 0.93
            ...
            if year >= 2012:
                industry_growth = 1 + 0.025 * (year - 2012)
                df.loc[i, 'Investissement_Industrie'] *= industry_growth
            
            # Développement agricole (spécifique à Saint-Louis)
            if year >= 2010:
            ...
            if year >= 2022:
                ...
                df.loc[i, 'Investissement_Industrie'] *= 1.14
                df.loc[i, 'Investissement_Commerce'] *= 1.13
    ...
    def create_financial_analysis(self, df):
        ...
        plt.suptitle(f'Analyse des Comptes Communaux de Saint-Louis ({self.start_year}-{self.end_year})', 
                    fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.savefig('saint_louis_financial_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _plot_sectorial_investments(self, df, ax):
        ...
        bottom = np.zeros(len(years))
        categories = ['Investissement_Agriculture', 'Investissement_Industrie', 
                     'Investissement_Commerce', 'Investissement_Education', 'Investissement_Transport']
        colors = ['#264653', '#2A9D8F', '#E76F51', '#F9A602', '#6A0572']
        labels = ['Agriculture', 'Industrie', 'Commerce', 'Éducation', 'Transport']
    ...
    def _generate_financial_insights(self, df):
        ...
        print(f"🏭 INSIGHTS ANALYTIQUES - Commune de Saint-Louis")
        print("=" * 60)
        ...
        # 5. Spécificités de Saint-Louis
        print("\n5. 🏭 SPÉCIFICITÉS COMMUNALES:")
        avg_industry_investment = df['Investissement_Industrie'].mean()
        ...
        print(f"Investissement industriel moyen: {avg_industry_investment:.2f} M€")
        ...
        print("• 2006-2007: Investissements dans les infrastructures industrielles")
        print("• 2008-2009: Impact de la crise financière mondiale")
        print("• 2010-2015: Développement des infrastructures commerciales et de transport")
        print("• 2020-2021: Impact de la crise COVID-19 sur l'économie locale")
        print("• 2022-2025: Plan de relance axé sur l'industrie et le commerce")
        
        # 7. Recommandations
        print("\n7. 💡 RECOMMANDATIONS STRATÉGIQUES:")
        print("• Capitaliser sur la diversité économique (agriculture, industrie, commerce)")
        print("• Développer les zones d'activités industrielles et commerciales")
        print("• Améliorer les infrastructures de transport (routes, transports en commun)")
        print("• Renforcer les services éducatifs et de formation professionnelle")
        print("• Maintenir un équilibre entre développement économique et protection environnementale")
...
def main():
    """Fonction principale"""
    print("🏭 ANALYSE DES COMPTES COMMUNAUX DE SAINT-LOUIS (2002-2025)")
    print("=" * 60)
    
    # Initialiser l'analyseur
    analyzer = SaintLouisFinanceAnalyzer()
    
    # Générer les données
    financial_data = analyzer.generate_financial_data()
    
    # Sauvegarder les données
    output_file = 'saint_louis_financial_data_2002_2025.csv'
    ...
    print(f"\n✅ Analyse des comptes communaux de Saint-Louis terminée!")
    print(f"📊 Période: {analyzer.start_year}-{analyzer.end_year}")
    print("📦 Données: Démographie, finances, investissements, dette")
    print("🏭 Spécificité: Focus sur les investissements industriels, agricoles et commerciaux")
