...
class SaintAndreFinanceAnalyzer:
    def __init__(self):
        self.commune = "Saint-André"
    ...
    def generate_financial_data(self):
        """Génère des données financières pour la commune de Saint-André"""
        print("🌾 Génération des données financières pour Saint-André...")
        ...
        # Données démographiques (commune importante et peuplée)
        ...
        # Investissements spécifiques (adaptés à Saint-André)
        ...
        # Ajouter des tendances spécifiques à Saint-André
        self._add_municipal_trends(df)
        
        return df
    
    def _simulate_population(self, dates):
        """Simule la population de Saint-André (commune importante)"""
    ...
    def _simulate_total_revenue(self, dates):
        ...
        base_revenue = 55  # millions d'euros en 2002 (budget important)
    ...
    def _simulate_state_grants(self, dates):
        ...
        base_grants = 20  # millions d'euros en 2002
        
        grants = []
        for i, date in enumerate(dates):
            # Dotations importantes pour commune rurale
            year = date.year
    ...
    def _simulate_operating_expenses(self, dates):
        ...
        base_operating = 32  # millions d'euros en 2002
    ...
    def _simulate_investment_expenses(self, dates):
        ...
        base_investment = 21  # millions d'euros en 2002
    ...
    def _simulate_debt_charges(self, dates):
        ...
        base_debt_charge = 4.5  # millions d'euros en 2002
    ...
    def _simulate_gross_savings(self, dates):
        ...
        for i, date in enumerate(dates):
            # Épargne modérée
            base_saving = 3.5  # millions d'euros en 2002
    ...
    def _simulate_agriculture_investment(self, dates):
        """Simule l'investissement agricole (spécifique à Saint-André)"""
    ...
    def _simulate_education_investment(self, dates):
        """Simule l'investissement éducatif (spécifique à Saint-André)"""
        base_investment = 5.0  # millions d'euros en 2002
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2007, 2012, 2017, 2022]:  # Investissements éducatifs
                multiplier = 1.7
            else:
                multiplier = 1.0
    ...
    def _simulate_roads_investment(self, dates):
        """Simule l'investissement routier (spécifique à Saint-André)"""
        base_investment = 4.0  # millions d'euros en 2002
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2005, 2010, 2015, 2020]:  # Gros investissements routiers
                multiplier = 1.9
            else:
                multiplier = 1.0
    ...
    def _simulate_health_investment(self, dates):
        ...
        for i, date in enumerate(dates):
            year = date.year
            if year in [2006, 2011, 2017, 2022]:  # Investissements en santé
                multiplier = 1.7
            else:
                multiplier = 1.0
    ...
    def _add_municipal_trends(self, df):
        """Ajoute des tendances municipales réalistes pour Saint-André"""
        for i, row in df.iterrows():
            ...
            # Développement accéléré (2010-2015)
                ...
                df.loc[i, 'Population'] *= 1.013  # Croissance démographique
            ...
            # Développement agricole (spécifique à Saint-André)
            if year >= 2010:
            ...
            if year >= 2022:
                ...
                df.loc[i, 'Investissement_Education'] *= 1.15
                df.loc[i, 'Investissement_Agriculture'] *= 1.14
    ...
    def create_financial_analysis(self, df):
        ...
        plt.suptitle(f'Analyse des Comptes Communaux de Saint-André ({self.start_year}-{self.end_year})', 
                    fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.savefig('saint_andre_financial_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _plot_sectorial_investments(self, df, ax):
        ...
        bottom = np.zeros(len(years))
        categories = ['Investissement_Agriculture', 'Investissement_Education', 
                     'Investissement_Routes', 'Investissement_Sante', 'Investissement_Culture']
        colors = ['#264653', '#2A9D8F', '#E76F51', '#F9A602', '#6A0572']
        labels = ['Agriculture', 'Éducation', 'Routes', 'Santé', 'Culture']
    ...
    def _generate_financial_insights(self, df):
        ...
        print(f"🌾 INSIGHTS ANALYTIQUES - Commune de Saint-André")
        print("=" * 60)
        ...
        education_share = (df['Investissement_Education'].mean() / df['Investissement'].mean()) * 100
        ...
        print(f"Part de l'éducation dans l'investissement: {education_share:.1f}%")
        
        # 6. Événements marquants
        print("\n6. 📅 ÉVÉNEMENTS MARQUANTS:")
        print("• 2002-2005: Développement initial de la commune")
        print("• 2006-2007: Investissements dans l'agriculture et les routes")
        print("• 2008-2009: Impact de la crise financière mondiale")
        print("• 2010-2015: Développement de l'éducation et de l'agriculture")
        print("• 2020-2021: Impact de la crise COVID-19 sur l'économie locale")
        print("• 2022-2025: Plan de relance axé sur l'éducation et l'agriculture")
        
        # 7. Recommandations
        print("\n7. 💡 RECOMMANDATIONS STRATÉGIQUES:")
        print("• Capitaliser sur les atouts agricoles (canne à sucre, fruits tropicaux)")
        print("• Développer les infrastructures éducatives (lycées, centres de formation)")
        print("• Améliorer les routes et les transports pour désenclaver les zones rurales")
        print("• Renforcer les services de santé pour une population croissante")
        print("• Diversifier l'économie locale au-delà de l'agriculture")
        print("• Préserver l'environnement et développer l'agriculture durable")
        print("• Développer le tourisme vert et les activités culturelles")

def main():
    """Fonction principale"""
    print("🌾 ANALYSE DES COMPTES COMMUNAUX DE SAINT-ANDRÉ (2002-2025)")
    print("=" * 60)
    
    # Initialiser l'analyseur
    analyzer = SaintAndreFinanceAnalyzer()
    
    # Générer les données
    financial_data = analyzer.generate_financial_data()
    
    # Sauvegarder les données
    output_file = 'saint_andre_financial_data_2002_2025.csv'
    ...
    print(f"\n✅ Analyse des comptes communaux de Saint-André terminée!")
    print(f"📊 Période: {analyzer.start_year}-{analyzer.end_year}")
    print("📦 Données: Démographie, finances, investissements, dette")
    print("🌾 Spécificité: Focus sur les investissements agricoles et éducatifs")
