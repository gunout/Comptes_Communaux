...
class SaintJosephFinanceAnalyzer:
    def __init__(self):
        self.commune = "Saint-Joseph"
    ...
    def generate_financial_data(self):
        """Génère des données financières pour la commune de Saint-Joseph"""
        print("🏛️ Génération des données financières pour Saint-Joseph...")
        ...
        # Données démographiques (Saint-Joseph est une commune moyenne)
        data['Population'] = self._simulate_population(dates)
        data['Menages'] = self._simulate_households(dates)
        
        # Recettes communales (budget moyen)
        ...
        # Investissements spécifiques (adaptés à Saint-Joseph)
        ...
        data['Investissement_Urbanisme'] = self._simulate_urbanism_investment(dates)
        
        df = pd.DataFrame(data)
        
        # Ajouter des tendances spécifiques à Saint-Joseph
        self._add_municipal_trends(df)
        
        return df
    
    def _simulate_population(self, dates):
        """Simule la population de Saint-Joseph (commune moyenne)"""
        base_population = 32000  # population estimée en 2002
    ...
    def _simulate_households(self, dates):
        ...
        base_households = 10500  # ménages en 2002
    ...
    def _simulate_state_grants(self, dates):
        ...
        base_grants = 18  # millions d'euros en 2002
    ...
    def _simulate_other_revenue(self, dates):
        ...
        base_other = 7  # millions d'euros en 2002
    ...
    def _simulate_total_expenses(self, dates):
        ...
        base_expenses = 42  # millions d'euros en 2002
    ...
    def _simulate_investment_expenses(self, dates):
        ...
        base_investment = 17  # millions d'euros en 2002
    ...
    def _simulate_debt_charges(self, dates):
        ...
        base_debt_charge = 3  # millions d'euros en 2002
    ...
    def _simulate_total_debt(self, dates):
        ...
        base_debt = 30  # millions d'euros en 2002
    ...
    def _simulate_agriculture_investment(self, dates):
        """Simule l'investissement agricole (spécifique à Saint-Joseph)"""
    ...
    def _simulate_tourism_investment(self, dates):
        """Simule l'investissement touristique (spécifique à Saint-Joseph)"""
        base_investment = 1.8  # millions d'euros en 2002
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2007, 2012, 2017, 2022]:  # Gros investissements touristiques
                multiplier = 1.6
            else:
                multiplier = 1.0
    ...
    def _simulate_transport_investment(self, dates):
        ...
        base_investment = 2.2  # millions d'euros en 2002
    ...
    def _simulate_urbanism_investment(self, dates):
        """Simule l'investissement en urbanisme (spécifique à Saint-Joseph)"""
        base_investment = 1.2  # millions d'euros en 2002
        
        investment = []
        for i, date in enumerate(dates):
            year = date.year
            if year in [2009, 2014, 2019, 2024]:  # Gros investissements en urbanisme
                multiplier = 1.9
            else:
                multiplier = 1.0
    ...
    def _add_municipal_trends(self, df):
        """Ajoute des tendances municipales réalistes pour Saint-Joseph"""
        for i, row in df.iterrows():
            ...
            if 2002 <= year <= 2005:
                ...
                df.loc[i, 'Investissement_Urbanisme'] *= 1.4
    ...
    def create_financial_analysis(self, df):
        ...
        plt.suptitle(f'Analyse des Comptes Communaux de Saint-Joseph ({self.start_year}-{self.end_year})', 
                    fontsize=16, fontweight='bold')
        plt.tight_layout()
        plt.savefig('saint_joseph_financial_analysis.png', dpi=300, bbox_inches='tight')
        plt.show()
    ...
    def _plot_sectorial_investments(self, df, ax):
        ...
        categories = ['Investissement_Agriculture', 'Investissement_Tourisme', 
                     ...
                     'Investissement_Sante', 'Investissement_Urbanisme']
        colors = ['#264653', '#2A9D8F', '#E76F51', '#F9A602', '#6A0572', '#5CAB7D']
        labels = ['Agriculture', 'Tourisme', 'Transport', 'Éducation', 'Santé', 'Urbanisme']
    ...
    def _generate_financial_insights(self, df):
        ...
        print(f"🏛️ INSIGHTS ANALYTIQUES - Commune de Saint-Joseph")
        print("=" * 60)
        ...
        # 5. Spécificités de Saint-Joseph
        print("\n5. 🌾 SPÉCIFICITÉS COMMUNALES:")
        avg_agriculture_investment = df['Investissement_Agriculture'].mean()
        avg_urbanism_investment = df['Investissement_Urbanisme'].mean()
        agriculture_share = (df['Investissement_Agriculture'].mean() / df['Investissement'].mean()) * 100
        urbanism_share = (df['Investissement_Urbanisme'].mean() / df['Investissement'].mean()) * 100
        
        print(f"Investissement agricole moyen: {avg_agriculture_investment:.2f} M€")
        print(f"Investissement urbanistique moyen: {avg_urbanism_investment:.2f} M€")
        print(f"Part de l'agriculture dans l'investissement: {agriculture_share:.1f}%")
        print(f"Part de l'urbanisme dans l'investissement: {urbanism_share:.1f}%")
        
        # 6. Événements marquants
        print("\n6. 📅 ÉVÉNEMENTS MARQUANTS:")
        print("• 2002-2005: Développement initial de la commune")
        print("• 2006-2007: Investissements dans l'agriculture et l'urbanisme")
        print("• 2008-2009: Impact de la crise financière mondiale")
        print("• 2010-2015: Développement des infrastructures de transport")
        print("• 2020-2021: Impact de la crise COVID-19 sur le tourisme")
        print("• 2022-2025: Plan de relance post-COVID axé sur le tourisme et l'urbanisme")
        
        # 7. Recommandations
        print("\n7. 💡 RECOMMANDATIONS STRATÉGIQUES:")
        print("• Capitaliser sur les atouts agricoles (fruits tropicaux, maraîchage)")
        print("• Développer le tourisme vert et les activités de pleine nature")
        print("• Améliorer les infrastructures routières et de transport")
        print("• Renforcer les services de santé et d'éducation")
        print("• Poursuivre les efforts d'aménagement urbain et de développement durable")
        print("• Diversifier l'économie locale au-delà de l'agriculture")
        print("• Adapter les services à la croissance démographique")

def main():
    """Fonction principale"""
    print("🏛️ ANALYSE DES COMPTES COMMUNAUX DE SAINT-JOSEPH (2002-2025)")
    print("=" * 60)
    
    # Initialiser l'analyseur
    analyzer = SaintJosephFinanceAnalyzer()
    
    # Générer les données
    financial_data = analyzer.generate_financial_data()
    
    # Sauvegarder les données
    output_file = 'saint_joseph_financial_data_2002_2025.csv'
    ...
    print(f"\n✅ Analyse des comptes communaux de Saint-Joseph terminée!")
    print(f"📊 Période: {analyzer.start_year}-{analyzer.end_year}")
    print("📦 Données: Démographie, finances, investissements, dette")
    print("🌿 Spécificité: Focus sur les investissements agricoles, touristiques et urbanistiques")
